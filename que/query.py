...
import dataclasses
import enum
from typing import List, Tuple, Union, NewType, Any, Dict, Mapping, Collection, Hashable, Type, \
    NamedTuple
...
ParamStyleType = NewType('ParamStyleType', Union[BasicParamStyle, NumParamStyle, NameParamStyle])
...
class Field:
    ...
    def __post_init__(self):
        try:
            assert self.name or self.value, \
    ...
    def for_fetch(self) -> str:
        ...
        return f"{self.name} AS {self.value}" if (self.value and self.name) else (self.name or self.value)
...
class Filter:
    ...
    def to_sql(self, args: 'ArgList' = None, style: ParamStyleType = DEFAULT_PARAM_STYLE) -> \
            Tuple[str, 'ArgList']:
...
class ArgList(FieldList):
    ...
    """
    def for_sql(self, style: ParamStyleType = DEFAULT_PARAM_STYLE) -> Union[Dict[str, Any], List[Any]]:
...
class _BaseWriteStatement(BaseSQLStatement):
    ...
    def __post_init__(self):
        ...
        try:
            ...
            assert columns and values and len(columns) == len(values), \
...
class Update(_BaseWriteStatement):
    ...
    def build_update(self, style: ParamStyleType = DEFAULT_PARAM_STYLE) -> Tuple[str, ArgList]:
...
def data_to_fields(data: FieldDataType, exclude: Any = Nothing) -> FieldList:
    ...
    if data:
        ...
        if dataclasses.is_dataclass(data):
            ...
            isinstance(data, Collection) and all((isinstance(x, Tuple) and len(x) == 2) for x in data)
        ):
